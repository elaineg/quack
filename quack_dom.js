((window) => {
	...
	if (e.key == "Escape") {
		...
		function send(params, callback) {
		    ...
		    xhr.open('GET', "https://morganscott.lib.id/QHActionCreate@dev/?" + Object.keys(params).map((k) => { return k + "=" + params[k]; }).join("&"), true);
		    xhr.send();
		}
		...
		send({
		    ...
		    entities: escape(JSON.stringify(q.entities)),
		    function_name: JSON.stringify(q).split("").map((c) => { return c.replace(/[^a-zA-Z]/, "a"); }).join("").slice(0, 20),
		}, (r) => {
		    var appID = r.id;
		})
